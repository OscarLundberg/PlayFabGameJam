handlers.list_lobby_events = function (args, context) {
    ...
    var lobby_events = JSON.parse(server.GetTitleData([args.Lobby]) || "[]");
...
handlers.send_lobby_event = function (args, context) {
    ...
    server.SetTitleData(args.Lobby, JSON.stringify(lobby_events));
    return { events: lobby_events };
}

handlers.create_lobby = function (args, context) {
    let lobbies = JSON.parse(server.GetTitleData([])["lobbies"] || "[]");
    console.log(lobbies);
    lobbies.push(args.Payload);
    server.SetTitleData("lobbies", JSON.stringify(lobbies));
    return { success: true };
}

handlers.get_lobbies = function (args, context) {
    return { lobbies: JSON.parse(server.GetTitleData(["lobbies"])) };
}
